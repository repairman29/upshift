...
**Modes (Blue / Yellow / Hot Rod):** See **[GETTING_STARTED_MODES.md](GETTING_STARTED_MODES.md)** — Blue = free + fallback (Groq → OpenRouter → Together), Yellow = premium fallback, Hot Rod = paid primary.  
**Free-tier fallbacks:** When Groq hits context overflow or rate limit (429), add **OpenRouter** and **Together** as fallbacks. See **[scripts/FREE_TIER_FALLBACKS.md](scripts/FREE_TIER_FALLBACKS.md)** for step-by-step: add `OPENROUTER_API_KEY` and `TOGETHER_API_KEY` to `.env` (or Vault), add the provider blocks and fallbacks to `clawdbot.json`, then restart the gateway.
